...
export default function AnimatedHero({ studentClass, firstName }: { studentClass: string | null; firstName: string }) {
  ...
  return (
    <section className="student-hero reference-hero">
      ...
      <div className="hero-stars" aria-hidden="true">{particles.map((i) => <i key={i} style={{ "--i": i } as React.CSSProperties} />)}</div>
...
.reference-hero .hero-grid{position:absolute;inset:0;opacity:.11;background-image:linear-gradient(rgba(167,139,250,.18) 1px,transparent 1px),linear-gradient(90deg,rgba(34,211,238,.12) 1px,transparent 1px);background-size:48px 48px;animation:gridMove 18s linear infinite}
.hero-stars{position:absolute;inset:0;overflow:hidden;pointer-events:none}.hero-stars i{position:absolute;left:calc((var(--i) * 37) % 100 * 1%);top:calc((var(--i) * 61) % 100 * 1%);width:2px;height:2px;border-radius:50%;background:#b9a7ff;opacity:.2;animation:twinkle calc(2.6s + (var(--i) * .17s)) ease-in-out infinite;animation-delay:calc(var(--i) * -.19s)}
...
@media(max-width:900px){.reference-hero{padding:50px 40px}.reference-stage{width:52%;right:-1%}}
@media(max-width:700px){.reference-hero{min-height:720px;padding:36px 24px 0;display:flex;flex-direction:column}.reference-hero .hero-copy{max-width:none}.reference-stage{position:relative;right:auto;top:auto;width:100%;max-width:none;height:360px;aspect-ratio:auto;transform:none!important;margin-top:auto}.reference-image{height:100%;width:100%}.hero-stars i:nth-child(n+15){display:none}}
@media(prefers-reduced-motion:reduce){.hero-grid,.hero-stars i,.reference-image,.art-aura,.energy-streams span,.hotspot:after,.hotspot,.scan-beam{animation:none!important}.reference-stage{transition:none}.reference-image{transform:none}}
`}</style>
    </section>
  );
}
